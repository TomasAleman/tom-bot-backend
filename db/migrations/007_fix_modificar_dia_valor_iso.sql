-- =====================================================================
-- Migration 007: parseo estable del dia al modificar reserva
-- =====================================================================
-- Si el cliente (n8n) envia p_valor como ISO con zona horaria
-- (ej. 2026-05-08T03:00:00.000Z), un cast directo ::DATE puede mover
-- el dia segun TimeZone de la sesion y dejar de coincidir con la lista
-- armada por fn_disponibilidad_modificar (siempre YYYY-MM-DD).
-- Se toma la parte fecha YYYY-MM-DD del prefijo cuando exista.
-- =====================================================================
...
BEGIN
    ...
    ELSIF p_campo IN ('personas', 'dia', 'horario') THEN
        ...
        -- mesas con alias: sin esto, restaurante_id choca con la columna
        -- OUTPUT de RETURNS TABLE en PL/pgSQL (ambiguedad).
        PERFORM 1
...
COMMENT ON FUNCTION tombot.fn_modificar_reserva IS
  'Modifica una reserva: nombre / personas / dia / horario. Dia: acepta YYYY-MM-DD o ISO con prefijo YYYY-MM-DD (ignora hora/TZ).';
